  ...
  <div class="content">
    <div class="container-fluid">
      ...
      <div class="row">
        ...
        <div class="col-md-12 <?php if (strpos($_SERVER['HTTP_HOST'], 'localhost') !== false){echo "d-block";}else{echo "d-none";} ?>">
          <div class="card">
        ...
        <div class="col-md-12">
          <div class="card input_area">
            ...
            <form id="langForm" method="post" class="validate-form" action="<?php echo base_url('admin/language/update_values/'.$type)?>" role="form" novalidate>

              <div class="row">
                <div class="col-sm-6">
                    <button type="submit" onclick="submitForm('update')" class="mb-10 btn btn-success pull-<?php echo($settings->dir == 'rtl') ? 'right' : 'left'; ?> m-t-20"><i class="fas fa-check-circle"></i> <?php echo trans('update-values') ?></button>
                </div>

                <div class="col-sm-6">
                    <div class="input-group mb-1">
                      <input type="text" name="search" class="form-control" placeholder="<?php echo trans('search-value') ?>">
                      <div class="input-group-append">
                        <button class="btn btn-primary" type="submit" onclick="submitForm('search')"><?php echo trans('search') ?> <i class="bi bi-search"></i></button>
                      </div>
                    </div>
                </div>
              </div>
...
function submitForm(action) {
    ...
    if (action === 'search') {
        form.action = '<?= base_url("admin/language/values/".$type.'/'.$value.'?search=true') ?>';
    } else if (action === 'update') {
        form.action = '<?= base_url("admin/language/update_values/".$type) ?>';
    }

    form.submit(); // now manually submit after setting action
}
</script>