...
<div class="dropdown-menu noti-card-position show_noti p-0 m-0">
  <?php if (empty($notifications)): ?>
    <div class="py-4 d-flex justify-content-center align-items-center">
      <p class="mb-0 text-muted"><?php echo trans('no-notifications-found') ?></p>
    </div>
  <?php else: ?>
    <?php foreach ($notifications as $notification): ?>
      ...
      <?php 
        ...
        if($notification->noti_type == 1){
          $noty_link = base_url('admin/users') ;
        ...
        }elseif($notification->noti_type == 4 ){
          $noty_link = base_url('admin/referral/payout_request') ;

        }elseif($notification->noti_type == 5 ){
          $noty_link = base_url('admin/domain/request');
          ...
          $noty_link = base_url('admin/appointment?type=all&action='.md5($notification->action_id));
        }elseif($notification->noti_type == 9){
          $noty_link = '#';
        }elseif($notification->noti_type == 10){
          $noty_link = base_url('admin/payouts/user');
        }elseif($notification->noti_type == 11){
          $noty_link = '#';
        }elseif($notification->noti_type == 12){
          $noty_link = base_url('admin/events/booking');
        }elseif($notification->noti_type == 13){
          $noty_link = base_url('admin/referral/payouts');
        }elseif($notification->noti_type == 14){
          $noty_link = base_url('admin/domain');
        }
      ...
      <a href="<?php echo html_escape($noty_link) ?>" class="dropdown-item <?php if($notification->seen == 0){echo 'bg-light';} ?>">
        <div class=" py-2 d-flex justify-content-start">
          <div>
            <div class="mt-1 avatar-xs" style="background-image: url(<?php echo base_url($thumb) ?>);"></div>
          </div>
          <div class="ml-4">
    ...
    <?php endforeach ?>
    <a href="<?php echo base_url('admin/notifications/all') ?>" class="py-3 dropdown-item dropdown-footer">See All</a>
  <?php endif ?>
</div>
