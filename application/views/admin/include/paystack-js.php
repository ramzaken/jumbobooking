<?php if (isset($booking_type) && $booking_type == 'Event'): ?>
      ...
      function payWithPaystack(){
        var handler = PaystackPop.setup({
          ...
          callback: function(response){
              window.location.href = `<?php echo base_url().'paystack/verify_customer_payment_event/' ?>${response.reference}/<?php echo $booking_id; ?>/<?php echo $amount; ?>`;                    
          },
          onClose: function(){
              alert('window closed');
          }
...
<?php elseif(isset($booking_type) && $booking_type == 'Product'): ?>
  <script>
    <?php if (isset($paystack_type) && $paystack_type == 'user'): ?>
      ...
      function payWithPaystack(){
        var handler = PaystackPop.setup({
          ...
          ref: ''+Math.floor((Math.random() * 1000000000) + 1), 
          callback: function(response){
              window.location.href = `<?php echo base_url().'paystack/verify_customer_payment_product/' ?>${response.reference}/<?php echo $type_id; ?>/<?php echo $amount; ?>`;                    
          },
          onClose: function(){
              alert('window closed');
          }
