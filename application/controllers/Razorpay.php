...
class Razorpay extends Home_Controller 
{
    ...
    // initialized cURL Request
    private function get_user_event_curl_handle($payment_id, $amount, $user_id)  {
        $user = $this->common_model->get_by_id($user_id, 'users');
    ...
    // callback method
    public function user_payment_event() {        
        
        if (!empty($this->input->post('razorpay_payment_id')) && !empty($this->input->post('merchant_order_id'))) {

            $event_booking = $this->common_model->get_by_id($this->input->post('booking_id'), 'event_booking');
            ...
            try {                
                ...
                $ch = $this->get_user_event_curl_handle($razorpay_payment_id, $amount, $event_booking->user_id);
                //execute post
                $result = curl_exec($ch);
                $http_status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                if ($result === false) {
            ...
            if ($success === false) {
                ...
                redirect(base_url('admin/payment/event_payment_success/'.$event_booking->id.'/razorpay'));
                
 
            } else {
                redirect(base_url('admin/payment/payment_cancel/'.$event_booking->id));
            }
        } else {
            redirect(base_url('admin/payment/payment_cancel/'.$event_booking->id));
        }
    ...
    public function user_payment_product() {        
        ...
        if (!empty($this->input->post('razorpay_payment_id')) && !empty($this->input->post('merchant_order_id'))) {
            ...
            try {                
                ...
                $ch = $this->get_user_curl_handle($razorpay_payment_id, $amount, $user->id);
                //execute post
                $result = curl_exec($ch);
                $http_status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                if ($result === false) {
            ...
            if ($success === false) {
                ...
                redirect(base_url('admin/payment/order_payment_success/'.$type_id.'/razorpay'));
                
 
            } else {
                redirect(base_url('admin/payment/payment_cancel/'.$type_id));
            }
        } else {
            redirect(base_url('admin/payment/payment_cancel/'.$type_id));
        }
