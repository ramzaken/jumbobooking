...
class Referral extends Home_Controller {
            ...
            if ($payout_amount >= $settings->minimum_payout && $payout_amount <= user()->referral_earn) {
               ...
               redirect(base_url('admin/referral/payouts'));
                ...
                $notify = array(
                    ...
                    'action_id' => 0,
                    'content_id' => 0,
                    'text' => user()->name. ' '.trans('make-affiliate-payout-request-of-').' $'.$amount.' '.trans('via'). ' '.$this->input->post('payment_method'),
                    'noti_type' => 4,
    ...
    public function payout_complete($id) 
    {
        ...
        $notify = array(
            ...
            'action_id' => 0,
            'content_id' => 0,
            'text' => trans('admin-accept-completed-your-affilitate-payout-request-of').' $'.$payout->amount.' '.trans('via').' '.$payout->payout_method,
            'noti_type' => 13,
            'noti_time' => my_date_now()
        );
        $notify = $this->security->xss_clean($notify);
