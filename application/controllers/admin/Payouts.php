...
class Payouts extends Home_Controller {
    ...
    public function complete($id) 
    {
        ...
        $notify = array(
            ...
            'action_id' => 0,
            'content_id' => 0,
            'text' => trans('admin-accept-completed-your-payout-request-of').' '.settings()->currency_symbol.($payout->amount/100).' '.trans('via').' '.$payout->payout_method,
            'noti_type' => 10,
        ...
        $calculate_amount = settings()->currency_symbol.($payout->amount/100);
        $user = $this->admin_model->get_by_id($payout->user_id,'users');

        $subject = get_email_by_slug('payout-confirm-admin')->subject;
        $body = get_email_by_slug('payout-confirm-admin')->body;
        $variables_data = [
    ...
    public function send_request()
    {
        ...
        if($_POST){   
            ...
            $notify = array(
                ...
                'action_id' => 0,
                'content_id' => 0,
                'text' => user()->name. ' '.trans('make-a-payout-request-of').' '.settings()->currency_symbol.($amount/100).' '.trans('via'). ' '.$payput_method,
                'noti_type' => 3,
                'noti_time' => my_date_now()
            );
            $notify = $this->security->xss_clean($notify);
            ...
            $calculate_amount = settings()->currency_symbol.($amount/100);

            $subject = get_email_by_slug('payout-request-admin')->subject;
            $body = get_email_by_slug('payout-request-admin')->body;
            $variables_data = [
                'user_name'  =>user()->name,
                'amount' => $calculate_amount,
                'payment_method' => $payput_method,
            ]; 
