...
class Language extends Home_Controller {
    ...
    public function update_values_old($type)
    {   
        check_status();
    ...
    public function update_values($type)
    {
        ...
        $language = $this->input->post('lang_type');
        $posted_data = $this->input->post(); // entire form data

        $data = [];

        // Loop only over posted keys that start with "value"
        foreach ($posted_data as $key => $val) {
            if (strpos($key, 'value') === 0) {
                // Extract ID from key (e.g. value123 => 123)
                $lang_id = str_replace('value', '', $key);

                // Build update data for that specific ID
                $data = [
                    $language => $val
                ];

                // Update only the matching lang ID
                $this->admin_model->edit_option($data, $lang_id, 'lang_values');
            }
        }
