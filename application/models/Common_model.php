...
class Common_model extends CI_Model {
    ...
    // select by slug
    function get_by_status_order($business_id, $table)
    {
        $this->db->select();
        $this->db->from($table);
        $this->db->where('business_id', $business_id);
    ...
    // get_payment
    function get_customer_appointments($total, $limit, $offset)
    {
    ...
    // get appointments
    function get_staff_appointments($total, $limit, $offset)
    {
    ...
    function today_appointments($times_later, $type)
    {
        $this->db->select('a.*, s.name as service_name, s.price, f.name as staff_name, c.name as customer_name, c.email as customer_email, c.phone as customer_phone');
        $this->db->from('appointments a');
        $this->db->join('services s', 's.id = a.service_id', 'LEFT');
        $this->db->join('staffs f', 'f.id = a.staff_id', 'LEFT');
        $this->db->join('customers c', 'c.id = a.customer_id', 'LEFT');
        $this->db->where('a.is_sent_reminder', 0);

        if ($type == 'hour') {

            $time_5_minutes_less = date("Y-m-d H:i", strtotime('-5 minutes', strtotime($times_later)));
            $this->db->group_start();
            $this->db->where('STR_TO_DATE(CONCAT(a.date, " ", SUBSTRING_INDEX(a.time, " - ", 1)), "%Y-%m-%d %H:%i") >=', date("Y-m-d H:i"));
            $this->db->where('STR_TO_DATE(CONCAT(a.date, " ", SUBSTRING_INDEX(a.time, " - ", 1)), "%Y-%m-%d %H:%i") >=', $time_5_minutes_less);
            $this->db->group_end();
        }

        if ($type == 'day') {
            $this->db->where('a.date', date("Y-m-d", strtotime('+'.$times_later.' day')));
        }
    ...
    public function update_email_sent_status($booking_id) {
        $this->db->where('id', $booking_id);
        $this->db->update('appointments', array('email_sent' => 1));
    }
...
// }
   ...
   function get_products($id, $total, $limit, $offset)
    ...
    function get_product_categories_by_company($business_id){
        $this->db->select();
     ...
     function get_product_subcategories_by_company($business_id){
        $this->db->select();
        $this->db->from('product_category');
        $this->db->where('parent_id !=', 0);
        $this->db->where('business_id', $business_id);
    ...
    function get_related_products($id,$business_id, $category_id, $limit)
    {
        $this->db->select('p.*');
