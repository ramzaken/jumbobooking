...
class Admin_model extends CI_Model {
    ...
    // select by function
    function select_by_order_user($table)
    {
        $this->db->select();
        $this->db->from($table);
        $this->db->where('user_id', $this->session->userdata('id'));
    ...
    function get_coupon_by_service($id)
    {
        $this->db->select();
    ...
    function check_coupon_apply($code, $service_id, $customer_id)
    {
        $this->db->select();
    ...
    function get_search_services($value)
    {
        $this->db->select();
        $this->db->from('services');
        $this->db->where('user_id',$this->session->userdata('id'));
    ...
    function get_search_customers($value,$disabled_customers)
    {
        $this->db->select();
        $this->db->from('customers');
        $this->db->where('user_id',$this->session->userdata('id'));
        $this->db->where_not_in('id', $disabled_customers);
        $this->db->like('name', $value);
    ...
    function get_appointments($user_id, $total, $limit, $offset)
    {
        ...
        if (isset($_GET['range']) && $_GET['range'] != 0) {
            ...
            $date_value = $_GET['range'];

            if ($date_value == '0') {
                // Fetch all appointments (no filtering)
            } elseif ($date_value == 'today') {
                // Fetch today's appointments
                $this->db->where('DATE(date)', date('Y-m-d'));
            } elseif ($date_value == 'tomorrow') {
                // Fetch tomorrow's appointments
                $this->db->where('DATE(date)', date('Y-m-d', strtotime('+1 day')));
            } elseif ($date_value == 'next_7_days') {
                // Fetch appointments in the next 7 days
                $this->db->where('DATE(date) >=', date('Y-m-d'));
                $this->db->where('DATE(date) <=', date('Y-m-d', strtotime('+7 days')));
            } elseif ($date_value == 'next_15_days') {
                // Fetch appointments in the next 15 days
                $this->db->where('DATE(date) >=', date('Y-m-d'));
                $this->db->where('DATE(date) <=', date('Y-m-d', strtotime('+15 days')));
            }
        }
        ...
        if(!empty($_GET['type']) && $_GET['type'] == 'upcoming'){
            $this->db->where('a.date < DATE_ADD(now(), INTERVAL 15 DAY) AND a.date > NOW()');
        }

        if(!empty($_GET['type']) && $_GET['type'] == 'pos'){
            $this->db->where('is_pos', 1);
        }
    ...
    function get_booking_customers_by_disabled($customers,$disabled_customers)
    {
        $this->db->select('a.*, c.name as name, c.email, c.phone, c.thumb');
        $this->db->from('appointments a');
        $this->db->where('a.user_id', $this->session->userdata('id'));
        $this->db->where_not_in('a.customer_id', $customers);
        $this->db->where_not_in('a.customer_id', $disabled_customers);
        $this->db->join('customers c', 'c.id = a.customer_id', 'LEFT');
        $this->db->group_by('a.customer_id');
        $query = $this->db->get();
        $query = $query->result();  
    ...
    function get_customers_by_disabled($disabled_customers)
    {
        $this->db->select('a.*, a.id as customer_id');
        $this->db->from('customers a');
        $this->db->where('a.user_id', $this->session->userdata('id'));
        $this->db->where_not_in('a.id', $disabled_customers);
        $this->db->order_by('a.id', 'DESC');
        $query = $this->db->get();
        $query = $query->result();  
        return $query;
    ...
    function get_pos_customers($disabled_customers)
    {
        $this->db->select('a.*, a.id as customer_id');
        $this->db->from('customers a');
        $this->db->where('a.user_id', $this->session->userdata('id'));
     ...
     function get_pos_payment($payment_id)
    {
        $this->db->select('p.*, a.number as booking_number, a.service_extra, a.date, a.time, c.name as customer_name, c.email as customer_email, c.phone as customer_phone, s.name as service_name, s.price as service_price');
        $this->db->from('payment_user as p');
    ...
    function get_time_by_days_staff($day_id, $business_id, $staff_id)
    {
        $this->db->select();
        $this->db->from('working_time');
        $this->db->where('day_id', $day_id);
    ...
    function check_time($time, $date, $service_id, $staff_id, $location_id)
    {
        ...
        $staff = get_by_id($staff_id,'staffs');
        $company = $this->admin_model->get_by_uid($staff->business_id, 'business');
    ...
    function get_break($business_id, $day)
    {
        $this->db->select();
        $this->db->from('working_time');
        $this->db->where('day_id', $day);
    ...
    function get_customer_event_payment_details($puid)
    {
        $this->db->select('p.*');
        $this->db->from('payment_user_event p');
        $this->db->join('event_booking e', 'e.id = p.event_id', 'LEFT');
        $this->db->where('p.puid', $puid);
        $query = $this->db->get();
        $query = $query->row();  
        return $query;
    ...
    function get_customer_product_payment_details($puid)
    {
        $this->db->select('p.*');
        $this->db->from('product_payment_user p');
        $this->db->where('p.puid', $puid);
        $query = $this->db->get();
        $query = $query->row(); 
        return $query;
    ...
    function get_customer_product_payment_details_orders($order_id)
    {
        $this->db->select('*');
    ...
    function get_customer_payment_lists($limit)
    {
        ...
        if ($this->session->userdata('role') == 'user') {
            ...
            $this->db->where('p.customer_id', $this->session->userdata('id'));
        }
    ...
    function get_customer_event_payment_lists($limit)
    {
        $this->db->select('p.*,');
        $this->db->from('payment_user_event p');
        $this->db->join('event_booking e', 'e.id = p.event_id', 'LEFT');
        ...
        $this->db->where('p.amount != ', '0.00');
        $this->db->group_by("p.event_booking_id");

        //$this->db->order_by('p.event_booking_id', 'DESC');
        if ($limit != 0) {
            $this->db->limit($limit);
        }
        $query = $this->db->get();
    ...
    function get_customer_order_payment_lists($limit)
    {
        $this->db->select('p.*,');
        $this->db->from('product_payment_user p');
    ...
    function check_duplicate_event_booking($ebook_number, $customer_id)
    {
        $this->db->select('*');
        $this->db->from('event_booking e');
        $this->db->where('e.booking_number', $ebook_number);
    ...
    function get_ebook_enumber_cus($ebook_number, $customer_id)
    {
        $this->db->select('*');
        $this->db->from('event_booking e');
        $this->db->where('e.booking_number', $ebook_number);
    ...
    function get_count_appointment_by_status($status)
    {
        $this->db->select('count(p.id) as total');
        $this->db->from('appointments p');
        if ($status != 'all' && $status != 'upcoming' && $status != 'pos') {
            $this->db->where('p.status', $status);
        }

        if ($status == 'upcoming') {
             $this->db->where('p.date < DATE_ADD(now(), INTERVAL 15 DAY) AND p.date > NOW()');
        }
        if ($status == 'pos') {
             $this->db->where('is_pos', 1);
        }
    ...
    function get_product_categories($user_id){
        $this->db->select();
    ...
    function get_product_subcategories($user_id){
        $this->db->select();
        $this->db->from('product_category');
        $this->db->where('user_id', $user_id);
        $this->db->where('parent_id !=', 0);
     ...
     function get_product($id, $total, $limit, $offset)
    {
        $this->db->select();
        $this->db->from('products');
        $this->db->where('business_id',$this->business->uid);
        if(!empty($_GET['product_search'])){
            $this->db->like('title',$_GET['product_search']);
        }
    ...
    function get_product_orders($total, $limit, $offset)
    {
        $this->db->select('p.*,');
        $this->db->from('product_orders p');
        $this->db->where('p.business_id',$this->business->uid);
        
        if(!empty($_GET['customer'])){
            $this->db->where('p.customer_id',$_GET['customer']);
        }

        if(!empty($_GET['order_status'])){
            $this->db->where('p.order_status',$_GET['order_status']);
        }
    ...
    // get_payment
    function check_event_payment($event_booking_id, $user_id, $customer_id)
    {
        $this->db->select('*');
        $this->db->from('payment_user_event');
        $this->db->where('user_id', $user_id);
        $this->db->where('event_booking_id', $event_booking_id);
        $this->db->where('customer_id', $customer_id);
        $this->db->order_by('event_booking_id', 'DESC');
        $query = $this->db->get();
        $query = $query->row();
        return $query;
    }
    ...
    function check_user_coupon_apply($user_id,$plan_id,$billing_type){
        $this->db->select();
        $this->db->from('plan_coupons_apply');
        //$this->db->where('coupon_id', $id);
        $this->db->where('user_id', $user_id);
        $this->db->where('plan_id', $plan_id);
        $this->db->where('plan_type', $billing_type);
        $query = $this->db->get();
        $query = $query->row();
        return $query;
    } 
    ...
    function get_language_by_status()
    {
        $this->db->select();
        $this->db->from('language');
        $this->db->where('status', 1);
    ...
    function get_lang_values_by_type($type)
    {
        ...
        if (isset($_GET['search']) && $_GET['search'] == 'true') {
            $this->db->like('label', $_POST['search']);
        }
        $this->db->order_by('id','DESC');
        $query = $this->db->get();
        $query = $query->result();
